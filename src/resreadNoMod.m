...
%{
    ...
    > flagEnd = 'MSC.NASTRAN' for 2012.1. Currently set for v2017
%} 
    ...
    if regexpi(card,'spcC')     chk = 1;    card = [repmat(' ',1,25) 'C O M P L E X   F O R C E S   O F   S I N G L E   P O I N T   C O N S T R A I N T'];	end
    if regexpi(card,'mpc')      chk = 1;    card = [repmat(' ',1,25) 'C O M P L E X   F O R C E S   O F   S I N G L E   P O I N T   C O N S T R A I N T'];	end
...
% Sort value, subcase number, pageval = Point/Freq/eigenfr.+modeNumber
	...
	subcase = cell2mat(textscan(join(A(blockEnd+off(2)+2,:)),'%*u%*s%f'));
...
% Assemble the blocks
    ...
    elseif chk==2
        subcs = unique(subcase);   
        for i = 1:length(subcs)    
            tmpID = unique(pageval(subcase==subcs(i),2));
            for j = 1:length(tmpID)
              	Q(:,:,j) = cell2mat(S(subcase==subcs(i) & pageval(:,2)==tmpID(j)));
            end
         	R{i}    = Q(:,2:7,:);
            dat{i}  = tmpID;
                clearvars Q 
